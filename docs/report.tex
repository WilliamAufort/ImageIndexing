...
\begin{definition}[(normalized) histogram]
	Let $w \in \mathcal{N}$ be a integer. Let $g: \mathcal{X} \to \RR^+$ be a granulometric function. We denote by $g_{max}$ the maximum value of $g$ over $\mathcal{X}$. We define the histogram of $g$ as the curve of the function $Hist(g): \set{0,\dots,w-1} \to \left[0,1\right]$ such that:

\[
	Hist(g)(n) = Card \left( \set{ x \in \mathcal{X}, \frac{n}{w}g_{max} < g(x) \leq \frac{n-1}{w}g_{max}} \right)
\]
\end{definition}

One can notice that we fix $w$ in this definition. In practize, we choose it not too small to observe something about the repartition of the data, but not too big also. Indeed, it induces more computations to have a very precise precise distribution we don't need. In our experiments, we have fixes $w = 20$, which seems to be a correct value with regard to our results.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[height=200pt]{images/granulo-noised}
