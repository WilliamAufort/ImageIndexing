...
As we will see in the further section, the time needed to compute this function is quite important, and it's really a mess to evaluate it on all the images of the database. % TODO Parler du temps total?
...
Let us suppose that $B(p,r) \nsubseteq B(p,DT(p))$ for some $p$ and $r$. Let $p'$ be the point of the border \widetilde{\mathcal{X}} such that $d(p,p') = DT(p)$. Let $q = p + (p'-p).\frac{r}{DT(p)}
...
Hopefully, we are not going to use this linear program, since we work in the particular case where the histograms are one-dimensional, and that's more they are also normalized (so $n = m$ and $(p1,\dots,p_n) = (q1,\dots,q_n) = (1,\dots,n)$). For example, in our case, the fourth condition became :
