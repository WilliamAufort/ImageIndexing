...
%\begin{figure}[!ht]
%    \centering
%    \includegraphics[height=175pt]{../database/shapedata.pgm}
%	\label{shapedata}
%	\caption{}
%\end{figure}
...
\begin{theo}[Correctness of Algorithm \ref{algo-naive}]
...
Basically, to compare two distributions, we can see them as a solution of a transportation problem. More precisely, it's the miniaml cost to transform one distribution to the other one. The definition consider distributions in $\RR^d$, but we only use it with $d=1$ (one-dimensional data).

\begin{defi}[Earth mover distance (general case)]
...
Hopefully, we are not going to use this linear program, since we work in the particular case where the histograms are one-dimensional, and that's more they are also normalized (so $n = m$ and $(p1,\dots,p_n) = (q1,\dots,q_n) = (1,\dots,n)$). For example, in our case, the fourth condition becames :

\[
	 \sum_{i=1}^m \sum_{j=1}^n f_{i,j} = \sum_{i=1}^m w_{p_i} = \sum_{j=1}^n w_{q_j}
\]

What's more, we have a more efficent way to compute it:

\begin{theo}[EMD in one dimension]
...
\]
	Then, we have: 
\[
	EMD(P,Q) = \sum_{i=1}^n{EMD_i}
\]
\end{theo}

\cite{EMD-proof} proposes an intuitive proof based on the interpretation of the quantity $\sum_{i=1}^n{EMD_i}$ in term of area between the two curves which represents the cumultative distributions of $P$ and $Q$.

% TODO : Figure du papier, attention à l'erreur de valeur

\subsubsection{Properties and advantages}

The Earth Mover Distance several advantages. First of all, it defines a metric in the mathematical sense:

\begin{theo}[EMD is a metric]
