...
To implement this, we use two \textsc{DGtal} primitives : \textsc{PowerMap} and \textsc{ReducedMedialAxis}. We don't enter into all the details, but we juste summurize the idea. The idea is to eliminate the balls which are completely included in another one, so we turn our initial problem into a combinatorial covering one, which can be solved using the power diagrams. 
...
But overall, these algorithms are not very fast, and it's a real problem when we are working in a database.
