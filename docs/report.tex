...
\begin{definition}[(normalized) histogram]
	Let $b \in \mathcal{N}$ be a integer (the number of bins of the histogram). Let $g: \mathcal{X} \to \RR^+$ be a granulometric function. We denote by $g_{max}$ the maximum value of $g$ over $\mathcal{X}$. We define the histogram of $g$ as the curve of the function $Hist(g): \set{0,\dots,w-1} \to \left[0,1\right]$ such that:
